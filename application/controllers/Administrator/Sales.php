...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            // Installment 
            if (isset($data->sales->isInstallment) && $data->sales->isInstallment == true) {
                $paymentDate = $data->sales->salesDate;
                for ($i = 1; $i <= $data->sales->installmentMonth; $i++) {
                    $date = date('Y-m-d', strtotime($paymentDate . ' + ' . $i . ' month'));
                    $installment = array(
                        'date'                 => date("Y-m-d"),
                        'installment_date'     => $date,
                        'installment_sales_id' => $salesId,
                        'installment_number'   => 'INS-0' . (strlen($i) < 10 ? '0' : '') . $i,
                        'installment_amount'   => $data->sales->installmentAmount,
                        'status'               => 'a',
                    ...
                    $this->db->insert('tbl_customer_installments', $installment);
                }
            }
    ...
    public function getSales()
    {
        ...
        if (isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $installments = $this->db->query("SELECT * FROM tbl_customer_installments WHERE installment_sales_id = ?", $data->salesId)->result();

            $res['installments'] = $installments;
        }
        $sales = $this->db->query("
            select 
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            $previousSaleMasterData = $this->db->query("select * from tbl_salesmaster where SaleMaster_SlNo = ?", $salesId)->row();
            ...
            if ($previousSaleMasterData->is_installment) {
                $this->db->query("delete from tbl_customer_installments where installment_sales_id = ?", $salesId);
            }
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            // installment query
            $installmentCount = $this->db->query("SELECT cp.*
            FROM tbl_customer_payment cp
            left join tbl_customer_installments ci on ci.installment_id = cp.installment_id
            WHERE cp.CPayment_status = 'a'
            and cp.CPayment_brunchid = ?
            and ci.installment_sales_id = ?", [$this->sbrunch, $saleId])->result();

            if (count($installmentCount) > 0) {
                $res = ['success' => false, 'message' => 'Unable to delete. Customer installment payment Founded'];
                echo json_encode($res);
                exit;
            }
            ...
            $this->db->set('status', 'd')->where('installment_sales_id', $saleId)->update('tbl_customer_installments');
