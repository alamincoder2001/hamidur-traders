...
class Products extends CI_Controller
{
    ...
    public function addProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and model = ? and color = ?", [$productObj->Product_Name, $productObj->model, $productObj->color])->num_rows();
    ...
    public function updateProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and model = ? and color = ? and Product_SlNo != ?", [$productObj->Product_Name, $productObj->model, $productObj->color, $productObj->Product_SlNo])->num_rows();
            if ($productNameCount > 0) {
                $res = ['success' => false, 'message' => 'Product name already exists'];
                echo json_encode($res);
                exit;
    ...
    public function getProducts()
    {
        ...
        $products = $this->db->query("
            select
                p.*,
                concat_ws(' - ',p.Product_Code, p.Product_Name, p.model, p.color) as display_text,
