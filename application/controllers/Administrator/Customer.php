...
class Customer extends CI_Controller
{
    ...
    public function getCustomerPayments()
    {
        ...
        $payments = $this->db->query("
            ...
            left join tbl_salesmaster sm on sm.SaleMaster_InvoiceNo = cp.CPayment_SaleInvoice
    ...
    public function monthly_payment_report()
    {
        ...
        $data['content'] = $this->load->view('Administrator/reports/monthly_payment_report', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getCustomerPaymentsMonthly()
    {
        $data = json_decode($this->input->raw_input_stream);
        ...
        $clauses = '';
        if (isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != '') {
            $clauses .= " and ci.installment_date BETWEEN '$data->dateFrom' AND '$data->dateTo'";
        }
        ...
        $result = $this->db->query("SELECT
            ...
            LEFT JOIN tbl_salesmaster sm on sm.SaleMaster_SlNo = ci.installment_sales_id
            LEFT JOIN tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
            WHERE ci.status = 'a'
            and ci.branch_id = '$this->cbrunch'
            -- and ci.installment_date <= '$currentDate'
            $clauses
        ")->result();
        ...
        //         from tbl_customer_payment cp
        //         WHERE cp.CPayment_status = 'a'
        //         and cp.CPayment_customerID = c.Customer_SlNo
        ...
        //     WHERE c.status = 'a'
        //     and c.Customer_Type != 'G'
        //     and c.Customer_brunchid = 1
        // ")->result();

        echo json_encode($result);
    }

    public function sendSmsDueCustomers()
    {
        $data = json_decode($this->input->raw_input_stream);
        $currency = $this->session->userdata('Currency_Name');
        ...
        try {
            foreach ($data->customers as $key => $customer) {
                $sendToName = $customer->Customer_Name;
                $recipient = $customer->Customer_Mobile;
                $currentDue = $customer->dueAmount;

                $message = "Dear {$sendToName},\nYour Due Amount is {$currency} {$currentDue}. Please Pay your due at your earliest";
                $this->sms->sendSms($recipient, $message);
            }
    ...
    public function installmentReport()
    {
        ...
        $data['title'] = "Installment Report";
        $data['content'] = $this->load->view('Administrator/reports/installment_report', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function getCustomerInstallmentDue()
    {
        ...
        $invoices = $this->db->query("SELECT
            ...
            FROM tbl_salesmaster sm
            WHERE sm.Status = 'a'
            and sm.SalseCustomer_IDNo = ?
            and sm.SaleMaster_branchid = ?
            and sm.is_installment is true
        ", [$data->customerId, $this->session->userdata('BRANCHid')])->result();

        $invoices = array_map(function ($value) {
            $value->installments = $this->db->query("SELECT
                    ci.*,  
                    (
                        SELECT ifnull(sum(cp.CPayment_amount),0)
                        FROM tbl_customer_payment cp
                    ...
                    FROM tbl_customer_installments ci
                    WHERE ci.status = 'a'
                    and ci.installment_sales_id = ?
                    and ci.branch_id = ?
                ", [$value->SaleMaster_SlNo, $this->session->userdata('BRANCHid')])->result();

            return $value;
        }, $invoices);


        // $installments = $this->db->query("SELECT
        // ci.*,
        // sm.SaleMaster_InvoiceNo,
    ...
    public function addInstallmentPayment()
    {
        ...
        try {
            ...
            // $installmentAmount = $this->db->query("
            //     select 
            //         cp.*,
            //         (
            //             select 
            //                 ifnull(sum(cp2.CPayment_amount),0) 
            //             from tbl_customer_payment cp2
            //             where cp2.CPayment_customerID = cp.CPayment_customerID
            //             and cp2.CPayment_status = 'a'
            //             and cp2.parent_id = cp.CPayment_id
            //         )as paid,
            //         (
            //             select cp.CPayment_amount - paid
            //         )as due
            //     from tbl_customer_payment cp
            //     join tbl_customer c on c.Customer_SlNo = cp.CPayment_customerID
            //     where cp.CPayment_customerID = ?
            //     and cp.CPayment_status = 'p'
            //     and cp.CPayment_invoice = ?
            //     and cp.CPayment_brunchid = ?
            //     and cp.parent_id is null
            //     order by cp.CPayment_invoice asc
            // ", [$paymentObj->CPayment_customerID, $paymentObj->CPayment_invoice, $this->session->userdata('BRANCHid')])->row();

            // if ($installmentAmount->due == 0) {
            //     $this->db->query("update tbl_customer_payment set CPayment_status = 'a' where CPayment_invoice = ? and parent_id is null ", $paymentObj->CPayment_invoice);
            // }

            $res = ['success' => true, 'message' => 'Payment added successfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
